...
const
  ...
  ScreenSize80x160x16: TPhysicalScreenInfo =
    (Width:  80; Height: 160; Depth: TDisplayBitDepth.SixteenBits);
  ScreenSize128x128x16: TPhysicalScreenInfo =
    (Width: 128; Height: 128; Depth: TDisplayBitDepth.SixteenBits);
  ScreenSize128x16x16: TPhysicalScreenInfo =
    (Width: 128; Height: 160; Depth: TDisplayBitDepth.SixteenBits);
...
begin
  ...
  writecommand(ST7735_COLMOD,$05);
  busy_wait_us_32(10000);

  writecommand(ST7735_FRMCTR1,$00,$06,$03);
  busy_wait_us_32(10000);

  writecommand(ST7735_MADCTL,$40+ST7735_MADCTL_BGR);
  writecommand(ST7735_DISSET5,$15,$02);
  writecommand(ST7735_INVCTR,$00);
  writecommand(ST7735_PWCTR1,$02,$70);
  busy_wait_us_32(10000);
  writecommand(ST7735_PWCTR2,$05);
  writecommand(ST7735_PWCTR3,$01,$02);
  writecommand(ST7735_VMCTR1,$3C,$38);
  busy_wait_us_32(10000);
  writecommand(ST7735_PWCTR6,$11,$15);
...
begin
  ...
  case displayRotation of
    ...
    begin
      if PhysicalScreenInfo = ScreenSize80x160x16 then
      begin
        colStart := 24;
        rowStart := 0;
      end;
      ...
      WriteCommand(ST7735_MADCTL,ST7735_MADCTL_MX + ST7735_MADCTL_MY + ST7735_MADCTL_BGR);
    end;
    TDisplayRotation.Right: begin
      ...
      WriteCommand(ST7735_MADCTL,ST7735_MADCTL_MY + ST7735_MADCTL_MV + ST7735_MADCTL_BGR);
    end;
    ...
    begin
      ...
      FScreenWidth := PhysicalScreenInfo.Width;
      FScreenHeight := PhysicalScreenInfo.Height;
      WriteCommand(ST7735_MADCTL,ST7735_MADCTL_BGR);
    end;
    TDisplayRotation.Left: begin
      ...
      FScreenWidth := PhysicalScreenInfo.Height;
      FScreenHeight := PhysicalScreenInfo.Width;
      WriteCommand(ST7735_MADCTL,ST7735_MADCTL_MX + ST7735_MADCTL_MV + ST7735_MADCTL_BGR);
    end;
  end;
end;
...
procedure TST7735_SPI.drawPixel(const x,y : word; const color:TColor);
begin
  if (x >= ScreenWidth) or (y >= ScreenHeight) then
    exit;
...
procedure TST7735_SPI.WriteCommand(const command,param1: Byte);
var
  data : array[0..0] of byte;
begin
  data[0]:= command;
...
procedure TST7735_SPI.WriteCommand(const command,param1,param2: Byte);
var
...
procedure TST7735_SPI.WriteCommand(const command,param1,param2,param3: Byte);
var
...
procedure TST7735_SPI.WriteCommandWords(const command : Byte; const param1,param2: Word);
var
...
procedure TST7735_SPI.WriteData(var data: array of byte;Count:longInt=-1);
begin
  if count = -1 then
    count := High(data)+1;
